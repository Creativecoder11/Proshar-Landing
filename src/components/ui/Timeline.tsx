        ...
        {data.map((item, index) => (
          ...
          `}
              ...
              <div className="flex flex-row items-center gap-4 md:pl-20">
                <div className={`${item.bgColor} px-3 pb-2 pt-1.5 rounded-sm`}>
                  <p className="text-xl font-medium text-white">
                    Step - {index + 1}
                  </p>
                </div>
                <h3 className="hidden md:block text-xl md:text-2xl font-semibold text-white">
                  {item.title}
                </h3>
              </div>
            </div>
            ...
            {/* Content */}
            <div className="pl-20 pr-4 md:pl-20 w-full text-[#F7F7F7]">
              {item.content}
              {index < data.length - 1 && (
                <div className="h-px my-6 pl-20 pr-4 md:pl-20 bg-white/20" />
              )}
            </div>
