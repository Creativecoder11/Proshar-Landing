export const FEATURES = [
  {
    title: 'Everyday Essentials',
    description: 'Keep your shelves full with the products your customers ask for every day. From staples to new finds.',
    icon: 'ledger',
  },
  {
    title: 'Trusted Origins',
    description: 'Authentic products from verified brands. No doubt, just genuine quality for your customers.',
    icon: 'wholesaler',
  },
  {
    title: 'Fair Pricing',
    description: 'Competitive rates that let you earn a fair margin on every sale. Sourcing shouldn\'t be expensive.',
    icon: 'monthlyreports',
  },
  {
    title: 'Quick Restocking',
    description: 'See what\'s available instantly. No guessing, no waiting. Restock in minutes, not hours.',
    icon: 'inventory',
  },
  {
    title: 'Business Support',
    description: 'Tools that help you manage your shop\'s finances and growth. We support your business journey.',
    icon: 'orderprocessing',
  },
  {
    title: 'Market Insights',
    description: 'Know what\'s selling in other lanes and keep your offer fresh. Stay ahead of local trends.',
    icon: 'analytics',
  },
];

export const STATS = [
  { value: '100%', suffix: '', label: 'Accuracy', description: 'Precision in every prescription' },
  { value: '5', suffix: 'x', label: 'Faster', description: 'Processing speed' },
  { value: '20', suffix: '%', label: 'Cost Reduction', description: 'Average savings' },
];

export const DETAILED_FEATURES = [
  {
    title: 'Easy Prescription Flow',
    icon: 'prescription',
    features: [
      'Seamless EHR integration',
      'Electronic signature capture',
      'Multi-pharmacy support',
      'Automated refill reminders',
    ],
  },
  {
    title: 'Leverage Data Portal',
    icon: 'data',
    features: [
      'Real-time analytics dashboard',
      'Custom report builder',
      'Export to multiple formats',
      'Scheduled report delivery',
    ],
  },
  {
    title: '2 Informer-Based Cross',
    icon: 'informer',
    features: [
      'Cross-reference drug interactions',
      'Clinical decision support',
      'Formulary compliance',
      'Patient-specific alerts',
    ],
  },
  {
    title: 'Better Database Updater',
    icon: 'database',
    features: [
      'Automatic drug updates',
      'NDC verification',
      'Price synchronization',
      'Compliance tracking',
    ],
  },
];

export const INTERFACE_FEATURES = [
  {
    title: 'Get It In Databiz',
    description: 'Comprehensive data integration for seamless business operations.',
    gradient: 'from-pink-500 to-purple-600',
  },
  {
    title: 'Vendor List Management',
    description: 'Organize and manage your vendor relationships in one place.',
    gradient: 'from-purple-600 to-blue-600',
  },
  {
    title: 'Sales & Track',
    description: 'Track sales metrics and performance in real-time.',
    gradient: 'from-emerald-500 to-cyan-600',
  },
];

export const SOCIAL_STATS = [
  { value: '1000+', label: 'Users' },
  { value: '50K+', label: 'Prescriptions' },
  { value: '24/7', label: 'Support' },
  { value: '4.9/5', label: 'Rating' },
];

export const TESTIMONIALS = [
  {
    quote: 'Proshar completely transformed how I manage my pharmacy. The Bengali ledger feature is perfect for my team, and the reports help me make better business decisions every day.',
    name: 'Khalid Hossain',
    role: 'Owner, Khalid Pharmacy',
    rating: 5,
    image: '/assets/logo/testimonial -3.jpg',
  },
  {
    quote: 'Managing suppliers used to be a nightmare. Now I can track all payments, dues, and purchase history in one place. The system saves me at least 10 hours every week!',
    name: 'Nafis Samee Raidah',
    role: 'Owner, DOHS Medicine',
    rating: 5,
    image: '/assets/logo/testimonial -2.jpg',
  },
  {
    quote: 'I run 3 pharmacy locations and Proshar handles them all seamlessly. The inventory alerts prevent stock-outs and the financial insights help me plan better.',
    name: 'Mahady Hasan',
    role: 'Owner, Dhaka Pharmacy',
    rating: 5,
    image: '/assets/logo/testimonial -1.jpg',
  },
];

export const FAQ_ITEMS = [
  {
    question: '01. What is Proshar?',
    answer: 'Proshar is the all-in-one B2B sourcing platform. We connect retailers directly with trusted brands and wholesalers, making sourcing simple, affordable, and efficient.',
  },
  {
    question: '02. Who is Proshar for?',
    answer: 'Proshar is built for every retailer who wants to streamline sourcing—from neighborhood shops to city stores, mini-marts, and professional sellers.',
  },
  {
    question: '03. How do I join Proshar?',
    answer: 'Simply create a free account, verify your business, and you\'ll get instant access to our catalog of authentic products and competitive pricing.',
  },
  {
    question: '04. Do you handle delivery?',
    answer: 'Yes. We are more than a platform; we are your supply line. We ensure your products reach your shop safely and on time.',
  },
  {
    question: '05. Can I manage my stock on the go?',
    answer: 'Absolutely. Proshar is designed for everyday readiness. Manage orders, track shipments, and check inventory from anywhere.',
  },
];

export const TRUST_BADGES = ['ISO', 'HIPAA', 'PCI DSS', 'GDPR', 'CCPA', 'SOC2', 'FDA'];

export const FOOTER_LINKS = {
  Product: ['Features', 'Pricing', 'Integrations', 'Testimonials'],
  Company: ['About Us', 'Careers', 'Contact', 'Blog'],
  Resources: ['Support', 'Documentation', 'API', 'Status'],
  Legal: ['Privacy Policy', 'Terms of Service', 'Cookies'],
};
